...
- if $\displaystyle  {\displaystyle (\forall \varepsilon >0)\,(\exists \delta >0)\,(\forall x\in \mathbb {R} )\,(0<|x-x_{0}|<\delta \implies |f(x)-L|<K\varepsilon )}$ where $K$ is K is a positive real number that does not depend on $\varepsilon$ or $x$
- then $\displaystyle\lim_{ x \to x_{0} }f(x)=L$ 
